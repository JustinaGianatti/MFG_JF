...
class Problem():
    ...
    def __init__(self, l, sub_l, C_l, L_l, g, L_g, A, L_A, B, L_B, C_B, T, N, dx, eps, C0, p, M0):
        ...
        C[0] = C0
        for i in range(N):
            C[i + 1] = C[i] + self.dt * max(C[i], self.C_alpha)
    ...
    def compute_P_opt_aux(self, V, M):
        def p_opt_k(x_index,y0_index):
            ...
            if abs(x_index-y0_index) > N_1_x:
                return 0
            else:
                sump = 0
                exponents = []
                for j in range(-N_1_x , N_1_x + 1):
                    y_aux = (x_index + j) * self.dx
                    ...
                    exponents.append(-(self.dt * self.l(((y_aux - xx) / self.dt), x_index, M) + V[x_index + j]) / self.eps)
            ...
            return math.exp(-(self.dt * self.l(((y0_index * self.dx - xx) / self.dt),x_index, M) +  V[y0_index]) / self.eps - min_exponent) / sump
        ...
        for k in range(self.N - 1, -1, -1):  # recorro la lista backward: N-1, N-2, ....,0
            ...
            for i in range(-self.N_list[k], self.N_list[k] + 1):
                ...
                for i_aux in range(-N_1_x_aux, N_1_x_aux + 1):
                    V_k[i] += p_opt_k(i, i + i_aux) * (self.dt * self.l((i_aux * self.dx / self.dt), i, M[k]) + V[0][i + i_aux])
            V.insert(0, V_k)
    ...
    def update_M(self, n, bar_M, BR_M):
        ...
        for k in range(self.N+1):
            M_k = np.zeros(2 * self.N_list[k] + 1)
            for i in range(-self.N_list[k], self.N_list[k] + 1):
                M_k[i] = (n / (n + 1)) * bar_M[k][i] + BR_M[k][i] / (n + 1)
            M.append(M_k)
        return M
