...
from MFG_auxiliary_SF import Problem, listToTuple
...
def M0(x):
    ...
    sum_aux_2 = 0
    for x2 in x2_grid:
        sum_aux_2 += math.exp(-((x2 + 0.1) ** 2) / 0.02)

    if abs(x) > C0:
        return 0
    else:
        return math.exp(-((x + 0.1) ** 2) / 0.02)  / sum_aux_2
...
def double_convolution(sigma,M):
    ...
    conv = np.convolve(rho, np.convolve(rho,M))  # longitud impar, ya que la conv de dos listas de N y M es N+M-1, y en nuestro casos todas las listas son impares
...
def l(alpha, x, M):
    ...
    return ((alpha)**2)/20 +  (x[1] * dx - 0.3) ** 2 + f_values[x[1] + len(M)]
...
def g(x, M):
    ...
    return (x[1] * dx - 0.3)**2 / 4 + f_values[x[1] + len(M)]
